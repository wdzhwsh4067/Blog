...
export const getToolboxData = async (databaseId) => {
  ...
  response.results.forEach((item: any) => {
    // through this command, fix a bug maybe caused by following command
    if (item.properties.Name && item.properties.Name.title[0] && item.properties.Name.title[0].plain_text) {
      results.push({
...
export const getChangelogData = async (databaseId) => {
  ...
  response.results.forEach((item: any) => {
    if (item.properties.Status && item.properties.Status.select && item.properties.Status.select.name) {
